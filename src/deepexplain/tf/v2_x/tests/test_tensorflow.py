...
def min_model(session):
    ...
    """
    X = tf.compat.v1.placeholder("float", [None, 2])
    out = tf.reduce_min(input_tensor=X, axis=1)
    session.run(tf.compat.v1.global_variables_initializer())
    return X, out
...
def min_model_2d(session):
    ...
    out = tf.reduce_min(input_tensor=tf.reshape(X, (-1, 4)), axis=1)
    session.run(tf.compat.v1.global_variables_initializer())
    return X, out
...
def train_xor(session):
    ...
    loss = tf.reduce_mean(
        input_tensor=tf.compat.v1.losses.mean_squared_error(Y, out))
...
class TestDeepExplainGeneralTF(TestCase):
    ...
    def test_tf_available(self):
        try:
            pkg_resources.require('tensorflow>=2.0.0')
        except Exception:
            try:
                pkg_resources.require('tensorflow-gpu>=2.0.0')
            except Exception:
    ...
    def test_context(self):
        ...
        for name in activations:
            ...
            x1_g = tf.gradients(ys=x1, xs=X)[0]
            self.assertEqual(
                x1_g.op.type,
        ...
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session):
            for name in activations:
    ...
    def test_invalid_method(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            self.assertIn('Method must be in', str(cm.exception))
    ...
    def test_T_is_tensor(self):
        ...
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            with self.assertRaises(RuntimeError) as cm:
                de.explain('grad*input', [Y], X, [[0, 0, 0]])
                self.assertIn(
                    'T must be a Tensorflow Tensor object', str(cm.exception))
    ...
    def test_T_has_compatible_batch_dim(self):
        ...
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            with self.assertRaises(RuntimeError) as cm:
                ...
                self.assertIn(
                    'the first dimension of the target tensor', str(cm.exception))
    ...
    def test_wrong_weight_len(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            with self.assertRaises(RuntimeError) as cm:
                ...
                self.assertIn(
                    'the number of elements in ys must equal ',
                    str(cm.exception)
                )
